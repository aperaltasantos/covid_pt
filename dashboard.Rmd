...
$$
Quanto maior for o valor do tempode duplicação, menor será a velocidade de progressão da transmissão.
...
Tal como um velocímetro, $R_{t}$ permite indicar quão rápido decorre a tranmissão (se $R_{t}$ estiver acima de 1, a transmissão está a decorrer a um ritmo elevado e a epidemia está a alargar-se), bem como se esta se encontra em aceleração ou desaceleração, ao longo de um determinado período do tempo (ou seja, se $R_{t}$ se está a afastar ou aproximar de 1, respectivamente).

O período temporal de cálculo utilizado abrange uma janela temporal de 7 dias. Exemplificando, o valor de $R_{t}$ reportado em 17-03-2020 diz respeito ao período temporal de 7 dias que termina nesse dia (11 a 17 de Março). Este elemento deve ser acutelado na interpretação do valor de $R_{t}$ como sendo reflexo de uma janela de transmissão que decorreu, para este cálculo concreto, nos últimos 7 dias.
